...
import { GameConflictError } from "../custom-errors";
...
describe("GameStateManager", () => {
    ...
    describe("getGame", () => {
        ...
        test("throws an error for a non-existent game ID", () => {
            expect(() =>
                gameStateManager.getGame("non-existent-game" as gameId)
            ).toThrow(GameConflictError);
        });
    });

    describe("createGame", () => {
        test("creates a new game with the given number of players", async () => {
            const newGame = await gameStateManager.createGame(
                4,
                "test-game",
                false,
                ""
        ...
        test("throws an error if a game with the ID already exists", async () => {
            ...
            await expect(
                gameStateManager.createGame(4, "test-game", false, "")
            ).rejects.toThrow(GameConflictError);
        });
    });
    ...
    describe("deleteGame", () => {
        test("deletes an existing game", async () => {
            ...
            expect(() =>
                gameStateManager.getGame(mockGame.id as gameId)
            ).toThrow(GameConflictError);
        });

        test("does not throw an error when deleting a non-existent game", () => {
            expect(() =>
                gameStateManager.deleteGame("non-existent-game" as gameId)
            ).not.toThrow();
        });
    });
